...
from api.services.yaml_dump_like_ansible import yaml_dump_like_ansible

from api.entities_not_in_bdd.nodes_and_groups import Group, Node, NodeOrGroup
...
class Inventory():
    ...
    def __init__(self, ansible_dir_path, inventory_file_subpath):
        ...
        if self.__inventory_file.init_error_return != "":
            self.init_error_return = self.__inventory_file.init_error_return
                ...
                for group_name, group_data in data["all"]["children"].items():
                    ...
                    if "hosts" in data["all"]["children"][group_name]:
                        for host_name, host_data in data["all"]["children"][group_name]["hosts"].items():
                            self.add_node(host_name, sync=False)
                            self.add_node_name_in_group_name(host_name, group_name, sync=False)
            ...
            for group_name, group in self.__groups_dict.items():
                if group.nodes_list != []:
                    ...
                    for node in group.nodes_list:
                        if data["all"]["children"][group_name] == None or not "hosts" in data["all"]["children"][group_name] :
...
#
    ...
    def get_node_by_name_vars_dict(self, node_name):
        return(self.__get_entity_by_name_vars_dict("node", group_name))
...
#
    def add_node_name_in_group_name(self, group_name, node_name, sync=True):
...
if __name__ == "__main__":
    inventory = Inventory("./", "./inventories/liberasys.yml")
    print(inventory.init_error_return)
